...
class ProjectsApi():
    ...
    def get_projects(self, hub_id: str, filterId: List[str]=None, filterName: List[str]=None, pageNumber: int=None, pageLimit: int=None):
        
        method = 'GET'
        local_url = '/project/v1/hubs/{}/projects'.format(hub_id)
        ...
        if pageNumber:
            query.update({'page[number]': pageNumber})
        if pageLimit:
            query.update({'page[limit]': pageLimit}) 
